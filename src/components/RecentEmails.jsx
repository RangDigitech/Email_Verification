...
const normalizeDetails = (dbRowOrJson = {}, listRow = {}) => {
  ...
  const fetchDetails = async (row) => {
    ...
    if (detailCache[row.id]) {
      setSelectedEmail(detailCache[row.id]);
      return;
    }
    setLoadingId(row.id);
    try {
      const r = await fetch(apiUrl(`/verifications/${row.id}`), {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        credentials: "include",
        cache: "no-cache",
      });
      if (r.ok) {
        const dbRow = await r.json();
        const normalized = normalizeDetails(dbRow, row);
        setDetailCache((m) => ({ ...m, [row.id]: normalized }));
        setSelectedEmail(normalized);
      } else {
        // Fallback: show whatever we have from the list
        setSelectedEmail(normalizeDetails({}, row));
      }
    } catch {
      setSelectedEmail(normalizeDetails({}, row));
    } finally {
  ...
  const handleEmailClick = (row) => {
    if (selectedEmail?.id === row.id) return;
    fetchDetails(row);
  };
          ...
          return (
            <div key={r.id}>
              ...
              >
                ...
                {/* Email */}
                <div
                  onClick={() => selectedEmail && handleBackClick()}
                  style={{
                    flex: 1,
                    fontSize: 15,
                    fontWeight: 500,
                    ...
                    whiteSpace: "nowrap",
                    color: "#fff",
                    cursor: selectedEmail ? "pointer" : "default",
                  }}
                >
                  {r.email}
                </div>
              ...
              {active && (
                <div style={{ padding: "24px 20px", background: "#181818", color: "#fff" }}>
...
function Section({ title, children }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: "#fff" }}>{title}</h3>
    ...
    <div style={{ width: "100%" }}>
      <div style={{ position: "relative", marginBottom: 8 }}>
        <div className="deliverability-meter">
          <div
            className="deliverability-meter-bar"
            style={{ width: "100%", height: 12, borderRadius: 6, position: "relative", overflow: "visible" }}
          >
            <div
              style={{
                position: "absolute",
      ...
      <div style={{ position: "relative", height: 24, marginTop: 8 }}>
        {markers.map((marker) => (
          <div
            key={marker.value}
            style={{ position: "absolute", left: `${marker.value}%`, transform: "translateX(-50%)", textAlign: "center" }}
          >
            <div style={{ fontSize: 12, fontWeight: 600, color: "#aaa" }}>{marker.label}</div>
          </div>
        ))}
      </div>
    </div>
